...
## h_min 스윕 17→28 km 확장 재실행 (2026-09-07)

- 질문 "23 km 위는 더 나쁜가"에 답하기 위해 sweep_hmin.py를 17~28 km 12점으로 확장
  (τ=0.2 s, n=200/조건, 통계 MC·밴드별 iid σ). 결과: 단조 악화 확인 — 보상 on 기준
  17 km 91.7 → 22 km 132.9 → 24 km 164.6 → 26 km 289.1 → 28 km 690.3 m (coast 구간
  증가가 지배, 25 km부터 가속). 고 h_min에서는 측정 수 자체가 적어 보상 on/off 차이도
  소멸(26 km: 289 vs 299).
- 절대값이 구버전(17 km 49.5, 22 km 75.8)보다 높아진 것은 P5 이후 시뮬 개선(직렬 촬영
  모델, 게이트 연쇄 기각 P 팽창 등)이 반영된 재실행이기 때문 — 현행 22 km 기준값이
  보정 통계 MC 110.8 m과 같은 자릿수로 정합. 결과 파일·그림 갱신:
  results/p5_hmin_tradeoff.json, figs/p5_hmin_tradeoff.png.
- 해석 유지: 이 곡선은 iid 통계 관점(낮을수록 유리, 17 km 최적)이고, 실런 검증의 저고도
  품질 붕괴(p5_sigma_vs_altitude)로 22 km 확정 — 확장분은 "위로 올려도 손해"라는 반대쪽
  경계를 채워 22 km가 양방향 트레이드오프의 선택임을 보여준다.

## 슬라이드 · 직렬 처리 모델 관련 내용 제외 (2026-09-07)

- 사용자 결정: 직렬 촬영 가정(처리 중 촬영 불가 → 드롭)은 아키텍처 조건부이므로
  슬라이드에서 제외. 정량 주장은 직렬/병렬이 비트 동일한 τ ≤ 프레임 주기 영역으로 한정.
- 변경: slide_10을 τ ≤ 주기만 그린 `figs/p7b_tau_scaling_slide.png`로 교체(전체판은
  문서·백업 유지), slide_06을 지연 보상 단독(`slide_06_delay_comp.png`)으로 재생성
  (직렬 패널 삭제), slide_12b(5 Hz 레이트 절벽 48.7 vs 65.7 m — 직렬 조건부) 슬라이드
  제외·12a는 `slide_12_jitter.png`로 개명.
- storyline 기여 ②·③ 재작성: ② 지연 보상 성립 조건 정량화(타임스탬프·유효 측정률·관성
  모델), ③ 양자화 = τ를 주기 아래로 유지하는 처리량 여유(처리량 상한 1/τ는 아키텍처
  불문, τ 수치로만 서술). handoff_ppt·PROMPTS·team_status의 구 킬러결과 문구도 동기화,
  qa_facts §8에 촬영 아키텍처 방어 항목 추가.
- 직렬 모델 자체는 문서(limitations §3)·results(p7b_rate_sweep, tau_serial τ>주기
  격자점)에 보존 — 삭제가 아니라 발표 범위 축소.

## P8 · 시연 세트를 실런 기준으로 전환 (2026-09-05)

- DemoPlayback에 궤적 선택(source: RealRun 기본/Truth, 폴백 지원). 실런 1비행
  (unity·wallclock·seed 0, 착륙 291.8 m·0.92 m/s — P8 MC 분포의 전형 표본)에서
  궤적(frames/traj_real.csv)+오버레이(frames/p6)를 동시 생성 → Display 2/3/4가
  같은 비행으로 일관. 캡처 506프레임×3 → 29.50 s 인코딩(목표 길이 기준 입력 fps
  자동 산출, 대회 30 s 규격). display4_telemetry.png는 실런 기준, truth는 _truth.
- 근거: truth 궤적 텔레메트리가 0으로 수렴하는 것이 "보여주기" 오해를 살 수 있다는
  사용자 지적 — 시연·수치 모두 실런 기준으로 정직화. 커밋 42b47df, 7d7259a.
...
- 핵심 수치(val 112프레임 / MC n=200·직렬·보상 on):
  - 탐지: 고전(고도 사전정보) mAP50-95 **0.093**·recall 0.256·τ **237.0 ms**
    vs YOLO11n INT8 **0.983**·0.999·**154.5 ms**(FP32 ONNX 동일 벤치 190.4 ms).
    사전정보 없는 전수 스케일은 mAP50 0.106·τ 528.2 ms — 미세 스케일 탐색이
    비용·오검출의 대부분. (τ는 FP32 추가 재벤치값, 재실행 간 수 % 지터)
