...
## P9 · 슬라이드 본문 제작 (발표 3분, 부록 무제한)

```
발표 시간은 3분이다(질의응답 별도). 본편은 3분에 말할 수 있는 최소 슬라이드만,
자세한 내용·근거·부차 결과는 전부 부록으로 뺀다(부록 분량 제한 없음). 슬라이드
파일 자체(pptx)는 만들지 않는다 — docs/slides.md에 슬라이드별 대본을 작성하면
사람이 슬라이드로 옮긴다.

읽고 시작: docs/storyline.md(15장 흐름·자산 매핑·금지 수치), docs/results_summary.md
(쓸 수 있는 숫자의 유일한 출처), docs/qa_facts.md(부록·Q&A 사실), figs/slides/
(그림·영상 자산 목록). @docs/ref/…정량화.pdf로 계획서 범위 확인.

산출물 docs/slides.md 구성:
- 본편(3분, 슬라이드 6~7장 목표 — 장당 발표 25~30초):
...
storyline.md의 사용 금지 수치(81.4/41.9/31 m 평탄/16.5 km 밴드/구 P7 곡선 등) 금지.
공개 저장소 규칙 — 팀명·소속·개인명·로컬 절대경로 금지(표지 팀 정보는 slides.md에
자리표시자 "[팀명]"으로만, 실제 값은 발표자가 로컬에서 채운다). 계획서 밖 주장 금지
(합성 렌더 학습은 신규성 아님, SLIM 55 m는 비교 기준 아님, MC 500회는 10월 본실험).

발표 대본 톤: 심사위원 대상, 각 슬라이드 첫 문장이 결론. 3분 안에 "왜 mAP로 부족한가
→ 우리가 뭘 쟀나 → 양자화가 실제로 착륙을 바꾸는 조건을 찾았다"가 전달되게 한다.
끝: STATUS.md 갱신, 커밋 "P9: slide script (3-min main + appendix)".
```
