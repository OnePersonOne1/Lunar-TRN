...
- 보상 성립 조건: 타임스탬프 1σ ≤20 ms 무해, 200 ms(≈τ)면 194.9 m로 미보상보다 악화,
  500 ms 349.5 m·게이트 붕괴. (p7b_jitter_sweep.json) 카메라 5 Hz에서 n INT8(48.7 m,
  드롭0) vs n FP32(65.7 m, 드롭50%) CI 분리 — 단, 이 수치는 직렬 촬영 모델 조건부라
  슬라이드 제외(§8 촬영 아키텍처 항목), 발표에서는 처리량 여유 framing만 사용.
  (p7b_rate_sweep.json)
...
- 기여 4가지: ① 공개 폐루프 테스트베드(인식→항법→유도, results 재현) ② INT8이 mAP
  ...
  들어오는 경로는 측정률·타임스탬프 오차·오검출률(양자화의 이득 = τ를 주기 아래로
  유지하는 처리량 여유)
...
- **촬영 아키텍처 가정 (직렬 처리는 슬라이드에서 제외, 2026-09-07)**: 시뮬은 직렬
  (처리 중 촬영 불가, 유효율 f/⌈τ·f⌉)과 병렬(매 틱 촬영, τ만큼 늦게 도착) 두 모드를
  갖고, **τ ≤ 프레임 주기에서는 두 모드가 비트 동일** — 발표의 정량 결과(110.8 m 평탄,
  FP32/INT8, 고전 비교, P8 실런)는 전부 이 아키텍처 무관 영역에 있다. 직렬 가정이 결과를
  가르는 곳은 τ > 주기의 악화 기울기와 고레이트 절벽(5 Hz 48.7 vs 65.7 m — 직렬 조건부라
  슬라이드 제외, p7b_rate_sweep.json에 보존)뿐. 아키텍처와 무관하게 남는 사실: 단일
  처리기의 처리량 상한은 1/τ이므로 τ > 주기면 유효 측정률이 떨어진다(하락 폭은 구현
  의존 — 이중 버퍼링 등 중간 구조는 미모델, limitations §3). 실사례: DIMES는 3.75 s/장
  처리(직렬에 가까움), Mars 2020 LVS는 파이프라인. "실제는 병렬 아니냐"에는 "맞다,
  그래서 직렬은 보수적 경계이고 정량 주장은 두 모드가 일치하는 영역에서만 했다"로 답한다.

## 9. 산출물 대응 (연구계획서 예상 결과 ①~⑤)

| # | 산출물 | 상태 |
|---|---|---|
| ① | 라벨 자동 생성 합성 데이터셋 | ✅ 1000+297프레임, 배포 아카이브 v1 준비 완료(docs/dataset_card.md, SHA256 기록) — Drive 공개 링크 기입 대기 |
