...
def run_closed_loop(
    ...
    if use_ekf:
        ...
        tau_max = tau if tau is not None else tau_sampler.max_tau
    ...
    for k in range(n_steps):
        ...
        if use_ekf:
            ...
            if (k + 1) % spf == 0 and h_min <= x[2] <= h_max:  # TRN 고도 구간에서만 촬영
                ...
                if valid:
                    tau_k = tau if tau is not None else tau_sampler.sample()
